<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        <!-- Container START -->
            <div id="colres">
                <div class="disp">';
                    $query2 = mysqli_query($config, "SELECT institusi, nama, website, email, alamat, logo FROM tbl_instansi");
                    list($institusi, $nama, $website, $email, $alamat, $logo) = mysqli_fetch_array($query2);
                        echo '<img class="logodisp" src="./upload/'.$logo.'"/>';
                        echo '<h6 class="up">'.$institusi.'</h6>';
                        echo '<h5 class="up" id="nama">'.$nama.'</h5><br/>';
                        echo '<span id="website">Website : '.$website.' &nbsp; Email : '.$email.'</span><br/>';
            ...
            <div id="lead">
                <p>Dekan Fakultas Sains dan Teknik</p>
                <div style="height: 50px;"></div>';
                $query = mysqli_query($config, "SELECT kadin, nip FROM tbl_instansi");
                list($kadin,$nip) = mysqli_fetch_array($query);
                if(!empty($kadin)){
                    echo '<p class="lead">'.$kadin.'</p>';
                } else {
                    echo '<p class="lead"> -</p>';
                }
                if(!empty($nip)){
